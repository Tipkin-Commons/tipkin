...
{block name=page_title}Mes emprunts{/block}
	...
	<div class="col_12">
		...
		<div id="tabs">
			<ul class="tabs">
				...
				<li style="float: right;">
					<a href="#waiting-paiement">En attente de paiement</a>
				</li>
			</ul>
			<div class="tab-content" id="current">
				{foreach from=$listOfReservations item=reservation}
					 {if $reservation->getUserSubscriberId() == $currentUser->id() && strtotime($reservation->getDateEnd()) + (24*60*60) > $smarty.now && $reservation->getStateId() == PaiementStates::WAITING_VALIDATION }
					 	<div class="col_12 visible">
							...
							<div class="col_3">
								<a href="/users/member/{$reservation->getUserOwnerId()}">Profil de {$usersManager->get($reservation->getUserOwnerId())->getUsername()}</a>
							</div>
					 	</div>
					 {/if}
				{/foreach}
			...
			<div class="tab-content" id="validated">
				{foreach from=$listOfReservations item=reservation}
					 {if $reservation->getUserSubscriberId() == $currentUser->id() && strtotime($reservation->getDateEnd()) + (24*60*60) > $smarty.now && $reservation->getStateId() == PaiementStates::VALIDATED }
					 	<div class="col_12 visible">
					 		{assign var="announce" 	value=$announcementManager->get($reservation->getAnnouncementId())}
					 		{assign var="mainPhoto" value="{Announcement::ANNOUNCEMENT_DIRECTORY}{$announce->id()}/{Announcement::THUMBNAILS_PREFIX}{$announce->getPhotoMain()}"}
							{assign var="userPhoto" value="{$profilesManager->getByUserId($announce->getUserId())->getAvatar()}"}
			...
			<div class="tab-content" id="canceled">
				{foreach from=$listOfReservations item=reservation}
					 {if $reservation->getUserSubscriberId() == $currentUser->id() && $reservation->getStateId() == PaiementStates::CANCELED }
					 	<div class="col_12 visible">
					 		{assign var="announce" 	value=$announcementManager->get($reservation->getAnnouncementId())}
					 		{assign var="mainPhoto" value="{Announcement::ANNOUNCEMENT_DIRECTORY}{$announce->id()}/{Announcement::THUMBNAILS_PREFIX}{$announce->getPhotoMain()}"}
							{assign var="userPhoto" value="{$profilesManager->getByUserId($announce->getUserId())->getAvatar()}"}
							...
							<div class="col_3">
								<label>Le : <span class="date">{$reservation->getDateEnd()}</span></label>
								Pour <span class="date-option">{$reservation->getDateOption()}</span>
								<span class="date-end">{$reservation->getDateEnd()}</span>
								<label>Prix : <span>{$reservation->getPrice()} €</span></label>
							</div>
			...
			<div class="tab-content" id="waiting-paiement">
				{foreach from=$listOfReservations item=reservation}
					 {if $reservation->getStateId() == PaiementStates::WAITING_PAIEMENT && $reservation->getUserSubscriberId() == $currentUser->id()}
					 	<div class="col_12 visible">
					 		{assign var="announce" 	value=$announcementManager->get($reservation->getAnnouncementId())}
					 		{assign var="mainPhoto" value="{Announcement::ANNOUNCEMENT_DIRECTORY}{$announce->id()}/{Announcement::THUMBNAILS_PREFIX}{$announce->getPhotoMain()}"}
							{assign var="userPhoto" value="{$profilesManager->getByUserId($announce->getUserId())->getAvatar()}"}
							...
							<div class="col_3">
								<a href="/users/member/{$reservation->getUserOwnerId()}">Profil de {$usersManager->get($reservation->getUserOwnerId())->getUsername()}</a>
								<br /><br />
								<a href="/paiement/{$reservation->id()}">Payer l'acompte</a>
								<br /><br />
								<a href="/activities/reservations/delete/{$reservation->id()}">Supprimer</a>
							</div>
					 	</div>
					 {/if}
				{/foreach}
